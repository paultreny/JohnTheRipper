 ...
 *
 *   MEMDBG_EXTRA_SHOW_NONERRORS_ON_VALIDATE if this is defined, then
 *      a call to MemDbg_Validate() when there are NO memory left, and
 *      no memory problems, will still show 'something'. This is to know
 *      that the memory checking DID occur. If this is NOT defined, then
 *      the call to MemDbg_Validate() at program exit will show NOTHING
 *      if there are no leaks or problems.
 */
...
void MemDbg_Display(FILE *fp) {
	...
	fprintf(fp, "MEMDBG: allocation information:  total size %lu", mem_size);
#ifdef MEMDBG_EXTRA_CHECKS
...
MEMDBG_realloc(const void *ptr, size_t size, char *file, int line)
{
	...
	p = CLIENT_2_HDR(ptr);
	if (p->mdbg_fpst != MEMFPOST || memcmp(p->mdbg_hdr2->mdbg_fpst, cpMEMFPOST, 4)) {
...
{
	fprintf(stderr, "Memory fence_post error, using dangling pointer, memory already freed - %p - %s(%d)\n", p, file, line);
}
